...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
					...
					<TBODY>
						...
						<TR>
							...
							<TD width="95%" bgColor="#689ace" height="27">
								<P align="center">
									<FONT face="Arial" color="#ffffff" size="5">行程单</FONT>
								</P>
							</TD>
								...
								<TABLE style="BORDER-COLLAPSE: collapse" borderColor="#111111" cellSpacing="0" cellPadding="0" width="100%" border="0">
									<TBODY>
										<TR>
											<TD width="50%">
												<B>
													<SPAN style="FONT-SIZE: 10.5pt; FONT-FAMILY: Times New Roman">PNR号码</SPAN>
	...
	<xsl:template match="NORMAL">
		<TR>
			<TD colSpan="2">航空公司：<xsl:value-of select="AIRLINE"/>
			</TD>
		</TR>
		<TR>
			<TD width="50%">航班号：<xsl:value-of select="FLIGHTNO"/>
					<xsl:if test="CARRIER!=''">
					<br/>(承运:<xsl:value-of select="CARRIER"/>)
					</xsl:if>
			</TD>
			<TD width="50%">舱位：<xsl:value-of select="CLASS"/>
			</TD>
		</TR>
		<xsl:if test="count(LEGS/LEG)=1">
			<TR>
				<TD width="50%">起飞城市：<xsl:value-of select="LEGS/LEG/ORIGIN"/>
				</TD>
				<TD width="50%">到达城市：<xsl:value-of select="LEGS/LEG/DEST"/>
				</TD>
			</TR>
			<TR>
				<TD width="50%">起飞时间：<xsl:value-of select="DATE"/>
			...
			<TR>
				<TD width="50%">飞行时间：<xsl:value-of select="ELAPSEDTIME"/>
				</TD>
				<TD width="50%">定座状态：<xsl:value-of select="SEATSTATUS"/>
				</TD>
			</TR>
			<TR>
				<TD width="50%">餐食：<xsl:value-of select="LEGS/LEG/MEAL"/>
				</TD>
				<TD width="50%">机型：<xsl:value-of select="LEGS/LEG/PLANETYPE"/>
				</TD>
			</TR>
		</xsl:if>
		<xsl:if test="count(LEGS/LEG)>1">
			<TR>
				<TD width="50%">起飞城市：<xsl:value-of select="LEGS/LEG[1]/ORIGIN"/>
				</TD>
				<TD width="50%">到达城市：<xsl:value-of select="LEGS/LEG[last()]/DEST"/>
				</TD>
			</TR>
			<TR>
				<TD width="50%">起飞时间：<xsl:value-of select="DATE"/>
	...
	<xsl:template match="INFO">
		...
		<xsl:if test="count(LEGS/LEG)=1">
			...
			<TR>
				<TD width="50%">起飞时间：<xsl:value-of select="LEGS/LEG/DEPTIME"/>
				</TD>
				<TD width="50%">到达时间：<xsl:value-of select="LEGS/LEG/ARRTIME"/><xsl:value-of select="LEGS/LEG/ARRDATECHG"/>
				</TD>
			</TR>
			<TR>
				<TD width="50%">飞行时间：<xsl:value-of select="ELAPSEDTIME"/>
		...
		<xsl:if test="count(LEGS/LEG)>1">
			...
			<TR>
				<TD width="50%">起飞时间：<xsl:value-of select="LEGS/LEG[1]/DEPTIME"/>
				</TD>
				<TD width="50%">到达时间：<xsl:value-of select="LEGS/LEG[last()]/ARRTIME"/><xsl:value-of select="LEGS/LEG[last()]/ARRDATECHG"/>
				</TD>
			</TR>
			<TR>
				<TD width="50%">飞行时间：<xsl:value-of select="ELAPSEDTIME"/>
	...
	<xsl:template match="ARNK">
		...
		<TR>
			<TD width="50%">起飞城市：<xsl:value-of select="ORIGIN"/>
			</TD>
			<TD width="50%">到达城市：<xsl:value-of select="DEST"/>
			</TD>
		</TR>
		<TR>
			<TD width="50%">起飞时间：<xsl:value-of select="DEPTIME"/>
			</TD>
			<TD width="50%">到达时间：<xsl:value-of select="ARRTIME"/><xsl:value-of select="ARRDATECHG"/>
			</TD>
		</TR>
		<TR>
			<TD width="50%">飞行时间：<xsl:value-of select="ELAPSEDTIME"/>
	...
	<xsl:template match="OPEN">
		...
		<TR>
			<TD width="50%">餐食：<xsl:value-of select="MEAL"/>
			</TD>
			<TD width="50%">机型：<xsl:value-of select="PLANETYPE"/>
			</TD>
		</TR>
	</xsl:template>
	<xsl:template match="/Response/ITINERARY/AGENTINFO">
		<xsl:value-of select="NAME"/>
		<br/>
    电话：<xsl:value-of select="CONTACTINFO/PHONE"/>
		<br/>
    传真：<xsl:value-of select="CONTACTINFO/FAX"/>
		<br/>
