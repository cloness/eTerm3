...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
									...
									<TBODY>
										<TR>
											...
											<TD width="50%">
												...
												<SPAN style="FONT-SIZE: 10.5pt; FONT-FAMILY: Times New Roman"><script version="" language="JavaScript">
				var today1 = new Date()
									...
									<TABLE style="BORDER-COLLAPSE: collapse" borderColor="#111111" cellSpacing="0" cellPadding="0" width="100%" border="0">
										<TBODY>
											<TR>
												<TD width="100%">
													<P align="left">
														<FONT face="Fixedsys" size="2">
															<xsl:for-each select="Response/ITINERARY/NOTICE/LINE">
																<li>
																<xsl:value-of select="."/>
																</li>
															</xsl:for-each>
															<xsl:for-each select="Response/ITINERARY/CAT18">
																<li>
																<xsl:value-of select="."/>
																</li>
															</xsl:for-each>
	...
	<xsl:template match="NORMAL">
		<TR>
			<TD colSpan="2">AIRLINE COMPANY:<xsl:value-of select="AIRLINE"/>
			</TD>
		</TR>
		<TR>
			<TD width="50%">FLGHT NO:<xsl:value-of select="FLIGHTNO"/>
					<xsl:if test="CARRIER!=''">
					(Carrier:<xsl:value-of select="CARRIER"/>)
					</xsl:if>
			</TD>
			<TD width="50%">CLASS:<xsl:value-of select="CLASS"/>
			</TD>
		</TR>
		<xsl:if test="count(LEGS/LEG)=1">
			<TR>
				<TD width="50%">DEP CITY:<xsl:value-of select="LEGS/LEG/ORIGIN"/>
				</TD>
				<TD width="50%">ARR CITY:<xsl:value-of select="LEGS/LEG/DEST"/>
				</TD>
			</TR>
			<TR>
				<TD width="50%">DEP DATE/TIME:<xsl:value-of select="DATE"/>
					<xsl:value-of select="LEGS/LEG/DEPTIME"/>
				</TD>
				<TD width="50%">ARR DATE/TIME:
			...
			<TR>
				<TD width="50%">FLYING TIME:<xsl:value-of select="ELAPSEDTIME"/>
				</TD>
				<TD width="50%">STATUS:<xsl:value-of select="SEATSTATUS"/>
				</TD>
			</TR>
			<TR>
				<TD width="50%">MEAL:<xsl:value-of select="LEGS/LEG/MEAL"/>
				</TD>
				<TD width="50%">AIRCRAFT TYPE:<xsl:value-of select="LEGS/LEG/PLANETYPE"/>
				</TD>
			</TR>
		</xsl:if>
		<xsl:if test="count(LEGS/LEG)>1">
			<TR>
				<TD width="50%">DEP CITY:<xsl:value-of select="LEGS/LEG[1]/ORIGIN"/>
				</TD>
				<TD width="50%">ARR CITY:<xsl:value-of select="LEGS/LEG[last()]/DEST"/>
				</TD>
			</TR>
			<TR>
				<TD width="50%">DEP DATE/TIME:<xsl:value-of select="DATE"/>
			...
			<TR>
				<TD width="50%">MEAL:<xsl:value-of select="LEGS/LEG[1]/MEAL"/>
				</TD>
				<TD width="50%">AIRCRAFT TYPE:<xsl:value-of select="LEGS/LEG[last()]/PLANETYPE"/>
				</TD>
			</TR>
			<TR>
				<xsl:for-each select="LEGS/LEG">
					<TD colSpan="2">
						<table border="1" width="100%" cellspacing="1" style="border-collapse:collapse">
							<tr>
								<td width="74">DEP CITY</td>
								<td width="240">
									<xsl:value-of select="ORIGIN"/>
								</td>
								<td width="91">ARR CITY</td>
								<td>
									<xsl:value-of select="DEST"/>
								</td>
							</tr>
							<tr>
								<td width="74">DEP DATE/TIME</td>
								<td width="240">
									<xsl:value-of select="DEPTIME"/>
								</td>
								<td width="91">ARR DATE/TIME</td>
								<td>
									<xsl:value-of select="ARRTIME"/><xsl:value-of select="ARRDATECHG"/>
								</td>
							</tr>
							<tr>
								<td width="74">STAY TIME</td>
								<td width="240">00</td>
								<td width="91">AIRCRAFT TYPE</td>
								<td>
	...
	<xsl:template match="INFO">
		...
		<xsl:if test="count(LEGS/LEG)=1">
			...
			<TR>
				<TD width="50%">DEP DATE/TIME:<xsl:value-of select="LEGS/LEG/DEPTIME"/>
				</TD>
				<TD width="50%">ARR DATE/TIME:<xsl:value-of select="LEGS/LEG/ARRTIME"/><xsl:value-of select="LEGS/LEG/ARRDATECHG"/>
				</TD>
			</TR>
			<TR>
				<TD width="50%">FLYING TIME:<xsl:value-of select="ELAPSEDTIME"/>
		...
		<xsl:if test="count(LEGS/LEG)>1">
			...
			<TR>
				<TD width="50%">DEP DATE/TIME:<xsl:value-of select="LEGS/LEG[1]/DEPTIME"/>
				</TD>
				<TD width="50%">ARR DATE/TIME:<xsl:value-of select="LEGS/LEG[last()]/ARRTIME"/><xsl:value-of select="LEGS/LEG[last()]/ARRDATECHG"/>
				</TD>
			</TR>
			<TR>
				<TD width="50%">FLYING TIME:<xsl:value-of select="ELAPSEDTIME"/>
	...
	<xsl:template match="ARNK">
		...
		<TR>
			<TD width="50%">DEP CITY:<xsl:value-of select="ORIGIN"/>
			</TD>
			<TD width="50%">ARR CITY:<xsl:value-of select="DEST"/>
			</TD>
		</TR>
		<TR>
			<TD width="50%">DEP DATE/TIME:<xsl:value-of select="DEPTIME"/>
			</TD>
			<TD width="50%">ARR DATE/TIME:<xsl:value-of select="ARRTIME"/><xsl:value-of select="ARRDATECHG"/>
			</TD>
		</TR>
		<TR>
			<TD width="50%">FLYING TIME:<xsl:value-of select="ELAPSEDTIME"/>
	...
	<xsl:template match="OPEN">
		...
		<TR>
			<TD width="50%">MEAL:<xsl:value-of select="MEAL"/>
			</TD>
			<TD width="50%">AIRCRAFT TYPE:<xsl:value-of select="PLANETYPE"/>
			</TD>
		</TR>
	</xsl:template>
